vDataJSON['project'] = {
    ...
    "ClassList": {
        ...
        "Editor4JSON": {
            ...
            "JSCC_mod_date": "2017/06/01 18:17:08",
            ...
            "MethodCode": {
                ...
                "next": "if (this.current < (this.data.length-1)) {\n    this.current++;\n};\nthis.edit();",
                ...
                "exportData": "",
                "exportSchema": "",
                "getLocalStorageID4Name": "return pDBName.replace(/[^A-Za-z0-9]/g,\"_\");\n",
                "loadLS": "  var vDBID = this.aName;\n  var vDB = this.aData;\n  if (typeof(Storage) != \"undefined\") {\n    // Store\n    if (typeof(localStorage.getItem(vDBID)) !== undefined) {\n      console.log(\"JSON-DB '\"+vDBIDe+\"' try loading from Local Storage\");\n      var vJSONstring = localStorage.getItem(vDBID);\n\t  if (!vJSONstring) {\n        console.log(\"JSON-DB '\"+vDBID+\"' undefined in Local Storage.\\nSave default as JSON\");\n        localStorage.setItem(vDBID, JSON.stringify(vDB));\n\t  } else {\n        console.log(\"parse DB '\"+vDBID+\"') from LocalStorage JSONstring='\"+vJSONstring.substr(0,120)+\"...'\");\n        vDB = JSON.parse(vJSONstring);\n\t  }\n    } else {\n      console.log(\"JSON-DB '\"+vDBID+\"' is undefined in Local Storage.\\nSave default as JSON\");\n      localStorage.setItem(vDBID, JSON.stringify(vDB));\n    };\n  }\t else {\n    console.log(\"WARNING: Sorry, your browser does not support Local Storage of JSON Database. Use Firefox ...\");\n  };\n  return vDB;\n",
                "saveLS": "  var vDBID = this.aName;\n  var vDB = this.aData;\n  if (typeof(Storage) != \"undefined\") {\n    // Store\n    if (typeof(vDB) != undefined) {\n      console.log(\"JSON-DB '\"+vDBID+\"' is defined, JSONDB in  Local Storage\");\n      if (vDB) {\n        var vJSONstring = JSON.stringify(vDB)\n        console.log(\"saveLocalDB('\"+vDBID+\"') JSONstring='\"+vJSONstring.substr(0,120)+\"...'\");\n        localStorage.setItem(vDB,vJSONstring);\n      } else {\n        console.log(\"JSON is NOT defined\");\n      }\n    } else {\n      console.log(\"JSON is undefined\");\n    };\n  }\t else {\n    console.log(\"WARNING: Sorry, your browser does not support Local Storage of JSON Database. Use Firefox ...\");\n  }\n"
            },
            "MethodComment": {
                "init": "Comment for init",
